...
public class hospitalDatabase extends JFrame implements ActionListener {
	...
	//Loading Table Data
	public static void loadHospitalData(DefaultTableModel model) {
		try (BufferedReader reader = new BufferedReader(new FileReader("txtfiles/hospitalDetails.txt"))) {
			String line;
			while ((line = reader.readLine()) != null) {
				if (line.trim().isEmpty()) continue;
				String[] parts = line.split(" / ");
				String hospitalCode = parts[0].split(": ")[1];
				String hospitalName = parts[1].split(": ")[1];
				String hospitalAddress = parts[2].split(": ")[1];
				String hospitalPhoneNumber = parts[3].split(": ")[1];
				model.addRow(new Object[]{hospitalCode, hospitalName, hospitalAddress, hospitalPhoneNumber});
			}
		} catch (IOException e) {
	...
	public hospitalDatabase() {
		...
		//Table Design
		String[] columns = {"Hospital Code", "Name", "Address", "Phone Number"};
		...
		loadHospitalData(model);
		add(new JScrollPane(table), "Center");

		//Function to Hide Button from Certain Users
